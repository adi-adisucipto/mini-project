...
async function rollbackExpiredPoints() {
    ...
    const rejectedTransaction = await prisma_1.default.transaction.findMany({
        where: {
            statusPay: client_1.StatusPay.Rejected,
        },
        include: {
            point: true,
            coupon: true
    ...
    for (const transaction of rejectedTransaction) {
        try {
